   ...
   $ = jQuery = parent.jQuery 
	...
	EditArea.prototype.init= function(){
        ...
        /*var checker = function(){
            setTimeout(function(){
                t.check_line_selection(null, true);
            },13)
            
        }
        $(t.textarea).mousedown(function(){
            t.check_line_selection();
            var mu = function(){
                checker();
                $(t.textarea).unbind("mousemove",checker)
                $(t.textarea).unbind("mouseup",mu)
                $(t.textarea).unbind("click",checker)
            }
            
            $(t.textarea).mousemove(checker)
            $(t.textarea).click(checker)
            $(t.textarea).mouseup(mu)
        })
        $(t.textarea).blur(checker)
        $(t.textarea).keypress(checker).keydown(checker).keyup(checker)*/
	...
	EditArea.prototype.manage_size= function(onlyOneTime){
		...
		if(editAreas[this.id]["displayed"]==true && this.textareaFocused)
		{
			...
			if( !this.settings['word_wrap'] )
			{
				...
				if(this.isOpera && this.isOpera < 9.6 || this.isChrome){
					area_width=10000; 								
				}
