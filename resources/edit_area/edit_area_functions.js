	...
	EditArea.prototype.save = function(e){
		editArea.execCommand('saveFile');
		try{
		    e.cancelBubble = true;
	...
	EditArea.prototype.rename_file= function(params){
		var id = params.id
		var new_id= params.new_id
		var new_title = params.new_title
		var f  = this.files[id];
		if(! f)	return;
		var elem = document.getElementById(f.html_id);
		f['id'] = new_id
		f['title'] =new_title
		var html_id= 'tab_file_'+encodeURIComponent(new_id);
		this.filesIdAssoc[html_id]= new_id;
		f['html_id']= html_id;
		this.files[new_id] = f
		delete this.files[id];
		...
		elem.id= f['html_id'];
		new_title = '<strong class="edited">*</strong>'+ new_title
		if(this.curr_file == id) this.curr_file = new_id
		
		var theclass = f.edited ? 'class="edited"' : ''
		
		
		var close= "<img src=\""+ parent.editAreaLoader.baseURL +"images/close.gif\" title=\""+ this.get_translation('close_tab', 'word') +"\" onclick=\"editArea.execCommand('close_file', editArea.filesIdAssoc['"+ html_id +"']);return false;\" class=\"hidden\" onmouseover=\"this.className=''\" onmouseout=\"this.className='hidden'\" />";
		elem.innerHTML= "<a onclick=\"javascript:editArea.execCommand('switch_to_file', editArea.filesIdAssoc['"+ html_id +"']);\" selec=\"none\" "+theclass+"><b><span>"+ new_title + close +"</span></b></a>";	
